...
const cn = cssClassName('SE_PersonalForm')
import * as T from "prop-types";
import A_H from 'A_H'
import A_P from 'A_P'
import A_InputText from 'A_InputText'
import A_InputNumber from 'A_InputNumber'
import A_InputPhone from 'A_InputPhone'
import Header from './Header'
import A_Btn from 'A_Btn'
import Country from './Country'

class SE_PersonalForm extends Component {
  ...
  onFormSubmit = (e) => {
    ...
    const value = code + phone.replace(/\D/g, '')
  ...
  validate(values){
    ...
    for (let valueName in values) {
      ...
      switch(valueName){
        ...
        case 'phone':
          if(!value){
            errors[valueName] = 'Please fill phone number field'
          } else if (value.length < 10) {
            errors[valueName] = 'Looks like an invalid phone number'
          } else {
            errors[valueName] = ''
          }
          break;
  ...
  onCodeChange = (code, error) => {
    if (error)
      this.validate({code})

    let countryObj = countryCodes.find(e => e.dial_code.toLowerCase() === code)

    if(!countryObj){
      countryObj = countryCodes.find(e => e.dial_code.startsWith(code))
    }

    const country = (countryObj && countryObj.name) || ''
    this.setState({values: {...this.state.values, country, code}})
  }

  onCountrySubmit = (country, error) => {
    if (error)
      this.validate({country})
    let countryObj = countryCodes.find(e => e.name.toLowerCase() === country.toLowerCase())

    if(!countryObj){
      countryObj = countryCodes.find(e => e.name.toLowerCase().startsWith(country.toLowerCase()))
    }
    const code = (countryObj && countryObj.dial_code) || ''
    this.setState({values: {...this.state.values, country, code}, windowShow: false})
  }

  render() {
    const {handleSubmit, ...props} = this.props
    const {
      values: {country, code, phone},
      errors,
      submitted,
    } = this.state
              ...
              {submitted ? (
                <div className={cn('body')}>
                  ...
                  <div className={cn('text')}>
                    <A_P type='third'>Enter your phone number, so that once you complete registation we will be able to reward you. </A_P>
                  </div>
                  <div className={cn('final-btn')}>
                    <a href="https://humaniq.com" className={cn('window-link')}>Humaniq.com</a>
                  </div>
                </div>
              ) : (
                <div className={cn('body')}>
                  <A_H type='window'>Leave your phone number</A_H>
                  <div className={cn('text')}>
                    <A_P type='third'>Enter your phone number, so that once you complete registation we will be able to reward you. </A_P>
                  </div>
                  <A_InputText
                    ...
                    onFocus = {() => this.setState({windowShow: true})}
                    handleChange={() => {}}
                  />
                  <div className={cn('code-phone')}>
                    <div className={cn('code-phone-code')}>
                      <A_InputNumber
                        value={code}
                        onChange
                        label="Code"
                        ...
                        max={4}
                        error={errors.code}
                        handleChange={code => this.onCodeChange(code, errors.code)}
                      />
                    </div>
                    <div className={cn('code-phone-phone')}>
                      <A_InputPhone
                        value={phone}
                        onChange
                        label="Phone number"
                        error={errors.phone}
                        handleChange={text => this.onChange('phone', text, errors.phone)}
                      />
                    </div>
                  </div>
                </div>
