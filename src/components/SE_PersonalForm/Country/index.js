...
class SE_PersonalFormCountry extends Component {
  ...
  onSubmit = (name) => {
    ...
    let countryObj = countries.find(с => с.name.toLowerCase() === name.toLowerCase())
    if(!countryObj) {
      countryObj = countries.find(с => с.name.toLowerCase().startsWith(name.toLowerCase()))
    }
    let _contryName = countryObj.name
    if(!_contryName){
      _contryName = countries[0].name
    }
